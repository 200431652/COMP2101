...
#Cleaning the script by transferring the output command to distinct variables
myhostname=$(hostname)
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #find the interface name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #find a lan address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') #find the lan hostname using getent hosts and variable $lan_address
external_ip=$(curl -s icanhazip.com) #to get the external IP address from curl
external_name=$(getent hosts $external_ip| awk '{print $2}') #to get the external ip by looking into hosts file with means of $external_ip
router_address=$(ip r s default| cut -d ' ' -f 3) #to find a default router address
router_hostname=$(getent hosts $router_address|awk '{print $2}')
#to find router hostname using getent host and $router_address
cat <<EOF
Hostname        : $myhostname
